...
// router.get("/individual_division_wise_customer", (req, res) => {
//   const sql = "SELECT * from individual_acc";
...
router.get("/peryear_bo_account_open_count", (req, res) => {
  const sql = "SELECT c_opening_date from individual_acc";
  db.query(sql, (err, result) => {
    console.log(result);
    if (err) throw err;
    res.send(result);
...
router.post("/crm", (req, res) => {
  ...
  const sql =
    "INSERT INTO crm (introducer_id,crm_name,crm_dob,crm_nid,crm_address,crm_email,crm_mobile_num) values (?,?,?,?,?,?,?)";
...
router.put("/individual/:nid", (req, res) => {
  const { bo_id } = req.body;
  const nid = req.params.nid;
  console.log({ nid, bo_id });
  const sql = "UPDATE individual_acc SET bo_id = ? WHERE c_code = ?";
  db.query(sql, [bo_id, nid], (err, results) => {
    if (err) throw err;
    res.json({ message: "employee added." });
  });
});
